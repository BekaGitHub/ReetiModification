...
public class RightForeArm extends StickmanSwingParts {
  ...
  public Point getHandStartPosition() {
    return (mArm != null) ? new Point((int) mArm.getCurrentPoint().getX(),
        (int) mArm.getCurrentPoint().getY()) : new Point(0, 0);
  }
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (mUpperArm.mRightShoulder.mBody.mNeck.mHead.mStickman.setCharacterInvisible == true) {
      if (mUpperArm.mRightShoulder.mBody.mNeck.mHead.mStickman.fadeControler
          == true) //Added by Robbie
      {
        int fadeFactor =
            mUpperArm.mRightShoulder.mBody.mNeck.mHead.mStickman.mMouth.mShapeAnimationStep * 12;
        if (fadeFactor <= 24) {
          fadeFactor = 0;
        }
        ...
        int fadeFactor =
            (20 - mUpperArm.mRightShoulder.mBody.mNeck.mHead.mStickman.mMouth.mShapeAnimationStep)
                * 12;
        if (fadeFactor >= 216) {
          fadeFactor = 255;
        }
