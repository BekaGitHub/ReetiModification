  ...
  public void createShape() {
    ...
    switch (mShape) {
      ...
      case SPEAK:
        ...
        Point lowerBubbleSpikeStart = new Point(upperBubbleSpikeStart.x + 10,
            upperBubbleSpikeStart.y + 5);
        Point topSeam = new Point(upperBubbleSpikeStart.x + mLength / 2,
            upperBubbleSpikeStart.y - mHeight * 3);
        Point bottomSeam = new Point(upperBubbleSpikeStart.x + mLength / 2,
            upperBubbleSpikeStart.y + mHeight / 3);
        ...
        mBubble.quadTo((mStart.x + upperBubbleSpikeStart.x) / 2 + 10,
            (mStart.y + upperBubbleSpikeStart.y) / 2 + 10, upperBubbleSpikeStart.x,
            upperBubbleSpikeStart.y);
        mBubble.curveTo(upperBubbleSpikeStart.x - mLength / 3, upperBubbleSpikeStart.y - 10,
            topSeam.x - mLength, topSeam.y, topSeam.x, topSeam.y);
        mBubble.curveTo(topSeam.x + mLength, topSeam.y, bottomSeam.x + mLength, bottomSeam.y,
            bottomSeam.x, bottomSeam.y);
        mBubble.quadTo((lowerBubbleSpikeStart.x + bottomSeam.x) / 2,
            (lowerBubbleSpikeStart.y + bottomSeam.y) / 2 + 3, lowerBubbleSpikeStart.x,
            lowerBubbleSpikeStart.y);
        mBubble.quadTo((mStart.x + lowerBubbleSpikeStart.x) / 2 + 10,
            (mStart.y + lowerBubbleSpikeStart.y) / 2 + 10, mStart.x, mStart.y);
        break;
    }
  }
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (mShape != SHAPE.DEFAULT) {
      ...
      if (endB > startB) {
        ...
        attributedString
            .addAttribute(TextAttribute.FOREGROUND, new Color(92, 31, 32), startB, endB);
      }
      LineBreakMeasurer textBreak = new LineBreakMeasurer(attributedString.getIterator(),
          fontRenderContext);
      ...
      while (textBreak.getPosition() < mLength) {
        ...
        layout.draw(g2, mBubbleCenter.x - textCunkWidth / 2,
            mBubbleCenter.y + hOffset * lines - drawYOffset);
        lines++;
      }
    }
    ...
    if (mCurrentlySpokenText.isEmpty()) {
      g2.setColor(new Color(0, 0, 0,
          64 - (AnimatorSwing.sMAX_ANIM_STEPS - (new Double(mShapeAnimationStep)).intValue()) * 3));
    } else {
