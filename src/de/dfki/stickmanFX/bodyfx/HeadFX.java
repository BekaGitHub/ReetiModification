...
public class HeadFX extends StickmanFXParts {
  ...
  public HeadFX(StickmanFX sm) {
    ...
    mRightEar = new Path();
    mColoropacity = 1f;
    color = Color.rgb(242, 227, 217, mColoropacity);
//        color = Color.rgb(242, 227, 217, 1);
  ...
  public void calculate(int step) {
    ...
    mHead.getElements().add(
        new CubicCurveTo(mEarWidth, -mHalfHeight / 5, mSize.width, -mHalfHeight / 5, mSize.width,
            mHalfHeight));
    mHead.getElements()
        .add(new CubicCurveTo(mSize.width, 120, mEarWidth, 120, mEarWidth, mHalfHeight));
    ...
    mLeftEar.getElements().add(
        new CubicCurveTo(0, mSize.height / 2 - 10, 0, mSize.height / 2 + 10, 10,
            mSize.height / 2 + 10));
...
//
    ...
    mRightEar.getElements().add(
        new QuadCurveTo(mSize.width + 3, mSize.height / 2, mSize.width, mSize.height / 2 - 10));
    mRightEar.getElements().add(
        new CubicCurveTo(mSize.width + 10, mSize.height / 2 - 10, mSize.width + 10,
            mSize.height / 2 + 10, mSize.width, mSize.height / 2 + 10));
  ...
  public void update() {
    ...
    if (mStickmanFX.setCharacterInvisible == true) {
      if (mStickmanFX.fadeControler == true) //Added by Robbie
      {
        ...
        //color = Color.rgb(242, 227, 217, (fadeFactor * 100 / 255) / 100f); //fadeFactor Interval [0 - 1]
      } else {
    ...
    mHead.setFill(color);
    // ears
    mLeftEar.setFill(color);
    mRightEar.setFill(color);
    // draw outlines
    //head
    mHead.setStroke(color.darker());
    mHead.setStrokeWidth(2);
    // ears
    mLeftEar.setStroke(color.darker());
    mLeftEar.setStrokeWidth(2);
    mRightEar.setStroke(color.darker());
