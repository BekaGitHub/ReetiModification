...
//    public Color mMaleColor = Color.rgb(14, 134, 122, (240 * 100 / 255) / 100f);
  ...
  public Color mFemaleColor = Color
      .rgb(154, 83, 198, mColoropacity);    // The color is changed in paintComponent
  public Color mMaleColor = Color.rgb(14, 134, 122, mColoropacity);
  // used by RestButton.
  public float mColoropacityRest = (240 * 100 / 255) / 100f;
  public Color mFemaleColorRest = Color
      .rgb(154, 83, 198, mColoropacity);    // The color is changed in paintComponent
  ...
  public void calculate() {
    ...
    this.mLefShoulderPosition = mNeckFX.mHeadFX.mStickmanFX.mLeftShoulderFX
        .getLeftShoulderEndPosition();       //Added by Beka
    this.mRightShoulderPosition = mNeckFX.mHeadFX.mStickmanFX.mRightShoulderFX
        .getRightShoulderEndPosition();

    mFemaleBodyFront = new Path();
    mFemaleBodyFront.getElements().add(new MoveTo(mStart.x, mStart.y));
    mFemaleBodyFront.getElements()
        .add(new LineTo(mLefShoulderPosition.x + 1, mLefShoulderPosition.y + 1));
    mFemaleBodyFront.getElements().add(
        new QuadCurveTo(mStart.x, mHalfSizeY + mDrawOffset, mStart.x + mHalfSizeX,
            mSize.height + 10));
    mFemaleBodyFront.getElements().add(
        new CubicCurveTo(mStart.x + mHalfSizeX - 40, mSize.height - 10, mStart.x - mHalfSizeX + 40,
            mSize.height + 20, mStart.x - mHalfSizeX, mSize.height));
    mFemaleBodyFront.getElements().add(
        new QuadCurveTo(mStart.x, mHalfSizeY + mDrawOffset, mRightShoulderPosition.x,
            mRightShoulderPosition.y));

    mFemaleBodyLeft = new Path();
    mFemaleBodyLeft.getElements().add(new MoveTo(mStart.x, mStart.y));
    mFemaleBodyLeft.getElements()
        .add(new QuadCurveTo(mStart.x + mDrawOffset, mSize.height / 3 * 2, mStart.x, mSize.height));
    mFemaleBodyLeft.getElements().add(new LineTo(mStart.x - mHalfSizeX, mSize.height));
    mFemaleBodyLeft.getElements()
        .add(new QuadCurveTo(mStart.x, mHalfSizeY + mDrawOffset, mStart.x, mStart.y));

    mFemaleBodyRight = new Path();
    mFemaleBodyRight.getElements().add(new MoveTo(mStart.x, mStart.y));
    mFemaleBodyRight.getElements()
        .add(new QuadCurveTo(mStart.x - mDrawOffset, mSize.height / 3 * 2, mStart.x, mSize.height));
    ...
    mMaleBodyFront.getElements()
        .add(new LineTo(mLefShoulderPosition.x + 1, mLefShoulderPosition.y + 1));
    mMaleBodyFront.getElements().add(new QuadCurveTo(mStart.x, mHalfSizeY + mDrawOffset,
        mStart.x + mHalfSizeX - mDrawOffset - 10, mSize.height));
    mMaleBodyFront.getElements()
        .add(new LineTo(mStart.x - mHalfSizeX + mDrawOffset + 10, mSize.height));
    mMaleBodyFront.getElements().add(
        new QuadCurveTo(mStart.x, mHalfSizeY + mDrawOffset, mRightShoulderPosition.x,
            mRightShoulderPosition.y));

    mMaleBodyLeft = new Path();
    mMaleBodyLeft.getElements().add(new MoveTo(mStart.x, mStart.y));
    mMaleBodyLeft.getElements()
        .add(new QuadCurveTo(mStart.x + mDrawOffset, mSize.height / 3 * 2, mStart.x, mSize.height));
  ...
  public Point getLeftLegStartPostion() {
    if (mNeckFX.mHeadFX.mStickmanFX.mOrientation == StickmanFX.ORIENTATION.LEFT) {
      ...
      return new Point(mStart.x + mHalfSizeX - mDrawOffset - 20,
          (mNeckFX.mHeadFX.mStickmanFX.mType == Gender.TYPE.FEMALE) ? mSize.height + 3
              : mSize.height);
    }
  }
  ...
  public Point getRightLegStartPostion() {
    if (mNeckFX.mHeadFX.mStickmanFX.mOrientation == StickmanFX.ORIENTATION.RIGHT) {
      ...
      return new Point(mStart.x - mHalfSizeX + mDrawOffset + 20,
          (mNeckFX.mHeadFX.mStickmanFX.mType == Gender.TYPE.FEMALE) ? mSize.height + 5
              : mSize.height);
    }
  }
  ...
  public void update() {
    ...
    if (mNeckFX.mHeadFX.mStickmanFX.setCharacterInvisible == true) {
      if (mNeckFX.mHeadFX.mStickmanFX.fadeControler == true) //Added by Robbie
      {
        ...
        mFemaleColor = new Color(mFemaleColor.getRed(), mFemaleColor.getGreen(),
            mFemaleColor.getBlue(), (fadeFactor * 100 / 255) / 100f);
