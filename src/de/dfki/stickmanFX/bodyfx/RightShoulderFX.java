...
public class RightShoulderFX extends StickmanFXParts {
  ...
  public Point getRightShoulderEndPosition() {
    ...
    if (mRotation >= 0 && mRotation <= 90) {
      return (mShoulder != null) ? new Point(
          (int) (mShoulder.boundsInParentProperty().get().getMinX() + 2),
          (int) mShoulder.boundsInParentProperty().get().getMaxY() - 2) : new Point(0, 0);
    } else if (mRotation > 90 && mRotation <= 180) {
      return (mShoulder != null) ? new Point(
          (int) (mShoulder.boundsInParentProperty().get().getMinX()),
          (int) mShoulder.boundsInParentProperty().get().getMinY()) : new Point(0, 0);
    } else if (mRotation < 0 && mRotation >= -90) {
      return (mShoulder != null) ? new Point(
          (int) (mShoulder.boundsInParentProperty().get().getMaxX()),
          (int) mShoulder.boundsInParentProperty().get().getMaxY()) : new Point(0, 0);
    } else {
      return (mShoulder != null) ? new Point(
          (int) (mShoulder.boundsInParentProperty().get().getMaxX()),
          (int) mShoulder.boundsInParentProperty().get().getMinY()) : new Point(0, 0);
