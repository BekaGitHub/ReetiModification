...
public class AnimationLoaderFX {
  ...
  private static final Set<String> sAnimationSubPackages = new HashSet<>(
      Arrays.asList("headfx", "facefx", "gesturefx", "environmentfx"));
  ...
  public AnimationStickmanFX loadAnimation(AgentInterface sm, String name, int duration,
      boolean block) {
      ...
      for (Constructor con : constructors) {
        ...
        if (params.length == 3) {
          if (params[0].getSimpleName().equalsIgnoreCase("stickmanFX")
            ...
            a = (AnimationStickmanFX) c.getDeclaredConstructor(params)
  ...
  public EventAnimationFX loadEventAnimation(AgentInterface sm, String name, int duration,
      boolean block) {
      ...
      for (Constructor con : constructors) {
        ...
        if (params.length == 3) {
          if (params[0].getSimpleName().equalsIgnoreCase("stickmanFX")
              && params[1].getSimpleName().equalsIgnoreCase("int")
              && params[2].getSimpleName().equalsIgnoreCase("boolean")) {
            a = (EventAnimationFX) c.getDeclaredConstructor(params)
                .newInstance(sm, duration, block);
          }
        }
      }
