...
public class LeftEye3D extends Stickman3DParts {
  ...
  private Path createEllipsePath(Path startElipse, double centerX, double centerY, double radiusX,
      double radiusY, double rotate, Color eyeColor, Color borderColor) {
  ...
  private Path createBorder(Path startBorder) {
    ...
    startBorder.getElements()
        .add(new QuadCurveTo(mStart.x + 10, mStart.y - 13, mStart.x + 20, mStart.y));
    startBorder.getElements()
        .add(new QuadCurveTo(mStart.x + 10, mStart.y + 13, mStart.x, mStart.y));
