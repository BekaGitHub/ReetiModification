...
public class AnimationLoader3D {
  ...
  public AnimationStickman3D loadAnimation(AgentInterface sm, String name, int duration,
      boolean block, HashMap<String, String> extraParams) {
    ...
    try {
      ...
      for (Constructor con : constructors) {
        ...
        if (params.length == 3 && extraParams.size() == 0) {
          if (params[0].getSimpleName().equalsIgnoreCase("stickman3d")
            ...
            a = (AnimationStickman3D) c.getDeclaredConstructor(params)
      ...
      ((Stickman3D) sm).mLogger
          .severe("AnimationInterface \"" + name + "\" cannot be found in " + cp);
    }
  ...
  public AnimationStickman3D loadAnimation(AgentInterface sm, String name, int frequent,
      int actionDuration, boolean block) {
    AnimationStickman3D a = null;
      ...
      for (Constructor con : constructors) {
        ...
        if (params.length == 4) {
          if (params[0].getSimpleName().equalsIgnoreCase("stickman3d")
            ...
            a = (AnimationStickman3D) c.getDeclaredConstructor(params)
                .newInstance(sm, frequent, actionDuration, block);
          }
        }
  ...
  public EventAnimation3D loadEventAnimation(AgentInterface sm, String name, int duration,
      boolean block) {
      ...
      for (Constructor con : constructors) {
        ...
        if (params.length == 3) {
          if (params[0].getSimpleName().equalsIgnoreCase("stickman3d")
              && params[1].getSimpleName().equalsIgnoreCase("int")
              && params[2].getSimpleName().equalsIgnoreCase("boolean")) {
            a = (EventAnimation3D) c.getDeclaredConstructor(params)
                .newInstance(sm, duration, block);
          }
        }
      }
