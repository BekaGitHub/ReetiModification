...
//        sInstance = null;
  ...
  public void start(Stage stage) throws Exception {
    ...
    sSubscene = createSubSceneAndCamera(getmStickmanHBox(),
        getWidth() - controlPanel.getPrefWidth(), getHeight());
  ...
  public float getFullScreenScale() {
    return getHeight() / (float) de.dfki.stickmanFX.StickmanFX.mDefaultSize.height * sScale
        * STICKMAN_SIZE_FACTOR;
  }
    ...
    Platform.runLater(() ->
    {
      ...
      try {
        ...
        for (String key : stickamnsOnStage.get(stageIdentifier).getStickmanNames()) {
          Stickman3D sman3D = (Stickman3D) stickamnsOnStage.get(stageIdentifier)
              .getStickmanByKey(key);
...
//            sman3D.isFullScreen = true;
          ...
          addStickmanName(key);
          ((StickmanStageController) generalConfigStageRoot.getmStickmanStageController())
              .setStage3D(this);
        }
      } catch (Exception e) {
        e.printStackTrace();
  ...
  private HBox findStageBox(HBox stickmanBox) throws Exception {
    for (Node child : stickmanBox.getChildren()) {
      if (child instanceof SubScene && ((SubScene) child).getRoot().getId()
          .equals(STICKMAN_STAGE)) {
   ...
   */
  @Override
  public void addAgentToStage(String stageIdentifier, de.dfki.stickmanFX.StickmanFX sman)
      throws Exception {
    HBox stickmanBox;
