...
public class StickmanStageController extends AStickmanStageController implements ViewController {
  ...
  public void initialize() {
    ...
    WithoutPerlinNoise.getStylesheets()
        .add(this.getClass().getResource("RadioButtonCSS.css").toExternalForm());
    WithPerlinNoise.getStylesheets()
        .add(this.getClass().getResource("RadioButtonCSS.css").toExternalForm());
    ...
    {
      ((StickmanStage3D) mStickmanOnstage.getStageStickman())
          .clearStage(((StageRoom3D) mStickmanOnstage.getStageRoom()).CONFIG_STAGE);
...
//            cr.start();
  ...
  public void handleBG6() {
    String bgDefault = getClass().getClassLoader().getResource("Images/bgDefault.png")
        .toExternalForm();
  ...
  private void handleSave() {
    ...
    if (mStickmanOnstage.getmFilePath() != null) {
      filexml = new File(
          mStickmanOnstage.getmFilePath() + File.separator + "stickman3d" + File.separator
              + "stickman3d.xml");
    } else {
      try {
        filexml = new File(
            new File(".").getCanonicalPath() + File.separator + "stickman3d" + File.separator
                + "stickman3d.xml");
      } catch (IOException e) {
        e.printStackTrace();
      }
