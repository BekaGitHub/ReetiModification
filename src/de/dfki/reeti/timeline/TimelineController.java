...
public class TimelineController implements Initializable {
      ...
      protected Void call() throws Exception {
        ...
        while (timelinePos <= TIMELINEWIDTH && playButtonON) {
          for (Pose pose : sequenceBlock.poseList) {
            ...
            if (timeline.getTranslateX() == Converter
                .SecondToPixel(pose.getStartTime() + sOffset)) {
              startMotors(pose);
            }
            ...
            if (timeline.getTranslateX() >= Converter
                .SecondToPixel(sequence.getProperty().getDuration() + 2)) {
  ...
  private void startMotors(Pose pose) {
    ...
    reeti.rightEar((int) motorMovement.getRightEar(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.leftEar((int) motorMovement.getLeftEar(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.rightEyeLid((int) motorMovement.getRightEyeLid(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.leftEyeLid((int) motorMovement.getLeftEyeLid(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.rightEyeTilt((int) motorMovement.getRightEyeTilt(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.leftEyeTilt((int) motorMovement.getLeftEyeTilt(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.rightEyePan((int) motorMovement.getRightEyePan(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.leftEyePan((int) motorMovement.getLeftEyePan(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.neckPan((int) motorMovement.getNeckPan(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.neckRotat((int) motorMovement.getNeckRotat(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    ...
    reeti.rightLC((int) motorMovement.getRightLC(),
        pose.getDuration().getTimeToReachPose() + sOffset);
    reeti
        .topLip((int) motorMovement.getTopLip(), pose.getDuration().getTimeToReachPose() + sOffset);
    reeti.bottomLip((int) motorMovement.getBottomLip(),
        pose.getDuration().getTimeToReachPose() + sOffset);
  }
}
